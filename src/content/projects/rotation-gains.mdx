...
We evaluated both techniques in a mixed-methods study (n=31), compared with a 
<strong>Static</strong> constant-gain baseline (g=2.5) across a <em>head-pointing task</em> 
and a <em>180° rotation task</em>.
...
Notably, this control enabled an emergent behavior in which users deliberately reoriented the virtual view, 
a form of user-initiated redirected viewing we term <strong>ViewShift</strong>.
<strong>Adaptive</strong> reduced cognitive load but could occasionally mis-time against user intent.
